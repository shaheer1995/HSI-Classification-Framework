...
class S3KAIResNet(nn.Module):
    def __init__(self, band, classes, reduction,kernel_size):
        ...
        self.conv_se = nn.Sequential(
            nn.Conv3d(
                kernel_size, band // reduction, 1, padding=0, bias=True),
            nn.ReLU(inplace=True))
        self.conv_ex = nn.Conv3d(
            band // reduction, kernel_size, 1, padding=0, bias=True)
        self.softmax = nn.Softmax(dim=1)

        self.res_net1 = Residual(
            kernel_size,
            kernel_size, (1, 1, 7), (0, 0, 3),
            start_block=True)
        self.res_net2 = Residual(kernel_size, kernel_size,
                                 (1, 1, 7), (0, 0, 3))
        self.res_net3 = Residual(kernel_size, kernel_size,
                                 (3, 3, 1), (1, 1, 0))
        self.res_net4 = Residual(
            kernel_size,
            kernel_size, (3, 3, 1), (1, 1, 0),
            end_block=True)
